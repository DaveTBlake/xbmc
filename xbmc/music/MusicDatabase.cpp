...
bool CMusicDatabase::GetArtistsByWhere(const std::string& strBaseDir, const Filter &filter, CFileItemList& items, const SortDescription &sortDescription /* = SortDescription() */, bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    if (limitedInSQL)
    {
      extFilter.limit = DatabaseUtils::BuildLimitClauseOnly(sortDescription.limitEnd, sortDescription.limitStart);
      const std::shared_ptr<CSettings> settings = CServiceBroker::GetSettingsComponent()->GetSettings();
      if (settings->GetBool(CSettings::SETTING_MUSICLIBRARY_USEARTISTSORTNAME))
        sorting.sortAttributes = static_cast<SortAttribute>(sorting.sortAttributes | SortAttributeUseArtistSortName);
  ...
  CVariant& result, int& total, const SortDescription &sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    GetOrderFilter(MediaTypeArtist, sortDescription, extFilter);
  ...
  CVariant& result, int& total, const SortDescription &sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    // Field names in album same as song - idAlbum, strArtistDisp, strArtistSort, strGenres, iYear, bCompilation
...
void CMusicDatabase::GetOrderFilter(const std::string& type, const SortDescription& sorting, Filter& filter)
{
