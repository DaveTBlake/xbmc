                                                 ...
                                                 const CDatabase &db, const std::string &strType) const
{
  ...
  if (strType == "songs")
  {
    ...
    {
      if (m_operator == OPERATOR_LESS_THAN || m_operator == OPERATOR_BEFORE ||
          m_operator == OPERATOR_NOT_IN_THE_LAST)
      {
        if (param == "0")
          query = GetField(m_field, strType) + " IS NULL";
        else
          query =
              GetField(m_field, strType) + " IS NULL OR " + GetField(m_field, strType) + parameter;
      }
      else if (param == "0")
        query = GetField(m_field, strType) + " IS NOT NULL";
    }
  }
